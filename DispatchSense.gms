...
scalar Load /400/;
Parameters report(counter,*),repGen(counter,gen),count;
Table data(Gen,*)
...
model ECD /all/;
loop (counter,load=sum(gen,data(gen,'Pmin'))+((ord(counter)-1)/(card(counter)-1))*sum(gen,data(gen,'Pmax')-data(gen,'Pmin'));
